...
describe("My First Cypress Test Suit", function () {
  it("My First Test Case", function () {
    ...
    //if you want to get via cypress locators to the exact button click, but! it can change in the future
    //manually-to be strong in case of customization
    cy.get("@prodLocator")
      .find(".product")
      .eq(2)
      .contains("ADD TO CART")
      .click(); // third product got clicked here via hard coding i.e. going through index elements
    ...
    cy.get("@prodLocator")
      ...
      .each(($el, index, $list) => {
        ...
        if (vegText.includes("Cash")) {
          //.includes() method is used here to get the substring, if that String is present in the prod name
          ...
          cy.wrap($el)
            ...
            }); //thats why cy.wrap($el) method for $el is used here 
          //to complete the promise of resolving the promise of finding the element
        }
      });
    ...
    cy.get(".brand").then(function (
      logoElement // here we are completing the promise under then() with a function
    ) {
